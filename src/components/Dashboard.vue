...
<template>
  <div>
    ...
    <button @click="removeFilter()"
      class="px-2 py-1 font-medium tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-600 rounded-md hover:bg-indigo-500 focus:outline-none focus:bg-indigo-500">
      Remove Filter
    </button>
                ...
                <td class="px-6 py-4 border-b border-gray-200 whitespace-nowrap">
                  <div class="text-center">
                    ...
                    <input type="checkbox" :name="'status' + index"
                      class="w-5 h-5 text-indigo-600 rounded-md focus:ring-indigo-500" name="status" :checked="false"
                      v-on:change="(e) => {
                        if (e.target.checked) {
                          bulkStatusIds[index] = user.id
                        } else {
                          delete bulkStatusIds[index];
                        }
                      }">
                  </div>
                </td>
